	...
	public static void main(String[] args) {
		if (args.length == 2) {
			...
			try {
				...
				System.out.println("Error creating report file: " + args[1]);
				return;
			}

			try {
				runProcess("javac -d " + args[0].substring(0, args[0].lastIndexOf('/')) + " " + args[0]);
				runProcess("java -cp " + args[0].substring(0, args[0].lastIndexOf('/')) + " "
						+ args[0].substring(args[0].lastIndexOf('/') + 1, args[0].length() - 5));
				if (errors.length() > 0) {
